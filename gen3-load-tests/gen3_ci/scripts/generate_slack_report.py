...
def generate_slack_report():
    report_link = f"https://allure.ci.planx-pla.net/load-tests/{datetime.now().strftime('%Y%m%d')}/{os.getenv("RUN_NUM")}/{os.getenv("ATTEMPT_NUM")}/index.html"
    slack_report_json = {}
    ...
    summary_block = {
        ...
        "text": {
            "type": "mrkdwn",
            "text": f"{test_result_icons[test_result]} {test_result} *{os.environ['TEST_SUITE']}* load test run for {version} on :round_pushpin:*{os.getenv('NAMESPACE')}* (took :stopwatch: *{gh_duration} minutes*)",
        },
    }
