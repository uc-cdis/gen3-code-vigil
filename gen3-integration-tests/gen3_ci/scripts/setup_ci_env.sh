...
if [ "$setup_type" == "test-env-setup" ] ; then
    ...
    if [ "$sower_block" != "key not found" ]; then
        ...
        yq eval-all --inplace 'select(fileIndex == 0) as $externalSecrets | select(fileIndex == 1) * {"sower": .sower // {} | .externalSecrets = $externalSecrets }' <(yq eval '.sower.externalSecrets' "$ci_default_manifest_dir/sower.yaml") "$ci_default_manifest_values_yaml"
    fi
