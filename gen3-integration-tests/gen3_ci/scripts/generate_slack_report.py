...
def generate_slack_report():
    if os.getenv("IS_NIGHTLY_RUN") == "true":
        ...
        replay_report_link = f"https://allure.ci.planx-pla.net/nightly-run-{os.getenv('CI_ENV')}/{datetime.now().strftime('%Y%m%d')}-replay/index.html"
        ...
        replay_report_link = f"https://allure.ci.planx-pla.net/{os.getenv('REPO')}/{os.getenv('PR_NUM')}/{os.getenv('RUN_NUM')}/{os.getenv('ATTEMPT_NUM')}-replay/index.html"
