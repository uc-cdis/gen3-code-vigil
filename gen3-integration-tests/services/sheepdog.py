...
class GraphRecord:
    def __init__(self, node_name, category, submission_order, props):
        ...
        sheepdog_id: unique "id" property returned by sheepdog when submitting a record
...
class Sheepdog:
    ...
    def delete_all_records_in_test_project(self, peregrine):
    ...
    def delete_record(self, sheepdog_id, expected_status_code=200):
        if not sheepdog_id:
            raise Exception("Unable to delete record that has no sheepdog_id")
        return self.delete_records([sheepdog_id], expected_status_code)

    def delete_records(self, sheepdog_ids, expected_status_code=200):
        try:
            result = self.sdk.delete_records(
                self.program_name, self.project_code, sheepdog_ids
            )
        except requests.exceptions.HTTPError as e:
            if e.response.status_code != expected_status_code:
