...
class MetadataService(object):
    ...
    def get_aggregate_metadata(self, study_id):
        res = requests.get(
            f"{self.AGG_MDS_ENDPOINT}/guid/{study_id}",
            headers=pytest.auth_header,
        )
        assert res.status_code == 200
        return res.json()["gen3_discovery"]

    def create_metadata(self, study_id, study_json):
    ...
    def update_metadata(self, study_id, study_json):
    ...
    def delete_metadata(self, study_id):
