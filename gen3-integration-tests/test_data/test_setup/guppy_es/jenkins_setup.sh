...
if false; then
  gen3 es create toy_debug_1 index_data/toy_mapping.json
  curl -H 'Content-Type: application/x-ndjson' "${ESHOST}/toy_debug_1/thing/_bulk" --data-binary @index_data/toy_batch.ndjson
fi

gen3 es create jenkins_subject_1 index_data/subject_mapping.json
gen3 es alias jenkins_subject_1 jenkins_subject_alias
curl -H 'Content-Type: application/x-ndjson' "${ESHOST}/jenkins_subject_1/subject/_bulk" --data-binary @index_data/subject_batch.ndjson

gen3 es create jenkins_file_1 index_data/file_mapping.json
gen3 es alias jenkins_file_1 jenkins_file_alias
curl -H 'Content-Type: application/x-ndjson' "${ESHOST}/jenkins_file_1/file/_bulk" --data-binary @index_data/file_batch.ndjson

gen3 es create jenkins_configs_1 index_data/array_mapping.json
