...
class CoreMetadataPage(object):
    def __init__(self):
        ...
        self.CORE_METADATA_PAGE_CLASS = "//div[@class='core-metadata-page']"
        self.CORE_METADATA_PAGE_PICTURE_CLASS = (
            "//div[@class='core-metadata-page__picture']"
        )
        self.CORE_METADATA_PAGE_HEADER_CLASS = (
            "//div[@class='core-metadata-page__header']"
        )
        self.CORE_METADATA_PAGE_TABLE_CLASS = (
            "//div[@class='core-metadata-page__table']"
        )
    ...
    def goto_metadata_page(self, page: Page, guid):
        ...
        page.wait_for_selector(self.CORE_METADATA_PAGE_CLASS, state="visible")
        screenshot(page, "CoreMetadataPage")

    def verify_metadata_page_elements(self, page: Page):
        """Verify the elements on the page are visible"""
        if not page.locator(self.CORE_METADATA_PAGE_CLASS).is_visible():
            logger.error("core-metadata-page element is missing")
            raise
        if not page.locator(self.CORE_METADATA_PAGE_PICTURE_CLASS).is_visible():
            logger.error("core-metadata-page__picture element is missing")
            raise
        if not page.locator(self.CORE_METADATA_PAGE_HEADER_CLASS).is_visible():
            logger.error("ore-metadata-page__header element is missing")
            raise
        if not page.locator(self.CORE_METADATA_PAGE_TABLE_CLASS).is_visible():
