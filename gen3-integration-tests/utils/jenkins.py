...
class JenkinsJob(object):
    ...
    def get_build_info(self, build_number):
        response = requests.get(f'{self.job_url}/{build_number}/api/json', auth=self.auth)
        if response.status_code == 404:
    ...
    def get_console_output(self, build_number):
        response = requests.get(f'{self.job_url}/{build_number}/consoleText', auth=self.auth)
        return response.text

    def build_job(self, parameters=None):
        url = f'{self.job_url}/build'
        if parameters:
            url += 'WithParameters?%s' % '&'.join(f'{k}={v}' for k, v in parameters.items())
    ...
    def get_artifact_content(self, build_number, artifact_name):
        try:
            artifacts = requests.get(f'{self.job_url}/{build_number}/api/json', auth=self.auth).json()['artifacts']
            artifact_rel_path = [d for d in artifacts if d['fileName'] == artifact_name][0]['relativePath']
            response = requests.get(f'{self.job_url}/{build_number}/artifact/{artifact_rel_path}', auth=self.auth)
            return response.text
