...
class TestPFBExport(object):
    ...
    def test_pfb_export(self, page):
        ...
        """
        url = "https://s3.amazonaws.com/dictionary-artifacts/anvil/master/schema.json"
        dictionary_response = requests.get(url)
        print(f"Dictionary {dictionary_response.status_code}")
        print(f"Dictionary {dictionary_response.json()}")
