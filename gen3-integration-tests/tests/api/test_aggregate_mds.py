...
class TestAggregateMDS:
    test_data_path = f'{Path(__file__).resolve().parent.parent.parent}/test_data/aggregate_mds'
    study_json_files = [
        'study1.json', 
        'study2.json', 
        'study3.json'
    ...
    def test_create_edit_delete_study_1(self):
        ...
        uid_field_name = portal_config.get('discoveryConfig', {}).get('minimalFieldMapping', {}).get('uid', None)
        ...
        for i in range(len(study_ids)):
            response = requests.post(f'{os.getenv("HOSTNAME")}/mds/metadata/{study_ids[i]}', data=json.dumps(study_jsons[i]), headers=auth_header)
        ...
        for i in range(len(study_ids)):
            ...
            assert study_metadata['project_title'] == study_jsons[i]['gen3_discovery']['project_title']

        # Edit metadata record
        logger.info('# Update metadata records')
        for i in range(len(study_ids)):
            project_title = study_jsons[i]['gen3_discovery']['project_title']
            study_jsons[i]['gen3_discovery']['project_title'] = f'{project_title} - Modified'
            # metadata.update(study_id, study_json)
            response = requests.put(f'{os.getenv("HOSTNAME")}/mds/metadata/{study_ids[i]}', data=json.dumps(study_jsons[i]), headers=auth_header)
            assert response.status_code == 200
        ...
        for i in range(len(study_ids)):
            response = requests.delete(f'{os.getenv("HOSTNAME")}/mds/metadata/{study_ids[i]}', headers=auth_header)
            assert response.status_code == 200
