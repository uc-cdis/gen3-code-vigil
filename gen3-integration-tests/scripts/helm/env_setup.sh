...
install_helm_chart() {
  ...
  if [ "$helm_branch" != "master" ]; then
    ...
    if helm upgrade --install gen3 gen3-helm/helm/gen3 --set global.hostname="${HOSTNAME}.planx-pla.net" -f helm_values/values.yaml -n "${NAMESPACE}"; then
    ...
    helm repo update
    if helm upgrade --install gen3 gen3/gen3 --set global.hostname="${HOSTNAME}.planx-pla.net" -f helm_values/values.yaml -n "${NAMESPACE}"; then
      echo "Helm chart installed!"
    else
      return 1
    fi
